...
public interface GameMembersApiService {
    ...
    Observable<List<Member>> listMembers(@Path(PATH_GAME_ID) String gameId);

    @POST(CREATE_MEMBER_URL)
    Observable<Member> createMember(
    ...
    Observable<Member> updateMember(
            @Path(PATH_GAME_ID) String gameId, @Path(PATH_USER_ID) String userId,
            @Body UpdateMemberDto updateMemberDto
    );

    @DELETE(DELETE_MEMBER_URL)
    Observable<Member> deleteMember(
            @Path(PATH_GAME_ID) String gameId, @Path(PATH_USER_ID) String userId
    );
}
